...
const ListItem = (props) => {
  ...
  useEffect(() => {
    ...
    getAllUsersDB(async (fetchedUsers) => {
        ...
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
          if (!querySnapshot.empty) {
            ...
            setUsers((currentUsers) => {
              ...
              return newUsers.sort((a, b) => b.lastMessageTimestamp?.seconds - a.lastMessageTimestamp?.seconds);
            });
          }
        });
      ...
      setUsers(usersWithSubscriptions.sort((a, b) => b.lastMessageTimestamp?.seconds - a.lastMessageTimestamp?.seconds));
    });
    ...
    <PageContainer title="" keyboardScroll={true}>
      ...
      >
        ...
        >
          <Image
            className="object-cover w-full h-full"
            source={{ uri: `https://ui-avatars.com/api/?name=GP&background=random&color=fff` }}
          />
        </View>
        <View className="flex-1">
          <Text style={{ fontSize: 15, fontWeight: 'bold', color: 'black', marginBottom: 4 }}>
